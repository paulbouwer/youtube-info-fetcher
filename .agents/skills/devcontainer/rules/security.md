...
```jsonc
// Bad: Prohibited without justification
{
  "privileged": true
}
```
...
Git refuses to operate on a workspace whose ownership differs from the executing user — a common case in DevContainers.

The `git-workflow` contributing rule (`$AGENT_SYSTEM_FOLDER/rules/git-workflow/devcontainer.md`) declares an `early` post-create hook that configures system-wide git safe-directory for the workspace. Any project loading that rule inherits the protection automatically.

**Risk** (if unaddressed): Without this configuration, git may refuse to operate due to ownership mismatches between host and container users.
...
// Good: Use specific paths with read-only where possible
{
  "mounts": [
...
- Review and restrict port forwarding to only ports required for development.
- Every port listed in `forwardPorts` **MUST** have a corresponding `portsAttributes` entry with at minimum a `label` and `onAutoForward` policy.

```jsonc
// Good: Only forward necessary ports — all with labels
{
...
**Risk**: Unnecessary port forwarding increases attack surface. **Risk**: Unlabelled ports appear as unnamed entries in VS Code's Ports panel, making it difficult for developers to identify each port's purpose.
